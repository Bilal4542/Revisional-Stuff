...
const BMICalculator = () => {
  ...
  const calculateBmi = () => {
    ...
    if (unitSystem === 'metric') {
      // For metric system: weight in kg, height in meters
      // User inputs height in cm, convert to meters (100 cm = 1 meter)
      const heightInMeters = heightValue / 100;
      calculatedBmi = weightValue / (heightInMeters * heightInMeters);
    } else {
      // For imperial system: weight in lbs, height in inches
      calculatedBmi = (703 * weightValue) / (heightValue * heightValue);
    }
  ...
  const determineBmiCategory = (bmiValue, ageValue) => {
  ...
  const chartData = {
    labels: history.map(record => format(record.date, 'MMM d')).reverse(),
    datasets: [
      {
        label: 'BMI',
        data: history.map(record => record.bmi).reverse(),
      ...
      {
        label: 'Weight',
        data: history.map(record => record.weight).reverse(),
          ...
          <div>
            ...
            <div className="mt-6 flex space-x-3">
              <button
                ...
                className="flex-1 bg-blue-600 hover:bg-blue-700 text-white font-medium py-2 px-4 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 disabled:opacity-50 disabled:cursor-not-allowed"
